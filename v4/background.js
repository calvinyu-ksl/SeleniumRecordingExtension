...
function generateSeleniumBaseScript(options = {}, actionsToUse = null) {
    ...
    const scriptSleepInterval = (options && options.scriptSleepInterval != null) ? parseFloat(options.scriptSleepInterval) : 1;
...
"""`,
    ...
    console.log(`Background: [SCRIPT-GEN] Total actions: ${actions.length}, Hover actions: ${hoverCount}`);
    if (hoverCount > 0) {
        console.log(`Background: [SCRIPT-GEN] Hover actions:`, actions.filter(a => a && a.type === 'Hover'));
    }
    ...
    for (const action of actions) { // Add steps to output to allForOutput (HTML_Capture only as comments)
        if (action.type === 'ScreenRecordingStart') {
            ...
            if (action.type === 'Hover') {
                console.log(`Background: [SCRIPT-GEN] Adding Hover action to output:`, action);
            }
    ...
    const hoverInOutput = allForOutput.filter(item => item.kind === 'action' && item.data && item.data.type === 'Hover').length;
    console.log(`Background: [SCRIPT-GEN] allForOutput count: ${allForOutput.length}, Hover in output: ${hoverInOutput}`);
    ...
    for (let i = 0; i < allForOutput.length; i++) {//loop all actions 
        ...
        switch (action.type) {
            case 'Click': { // Click
                ...
                lines.push(`        try:`);
                lines.push(`            self.wait_for_element_present(${finalClickSelector}, timeout=5)`);
                lines.push(`            self.scroll_to(${finalClickSelector})`);
                lines.push(`        except Exception:`);
                lines.push(`            pass  # Continue even if scroll fails`);
                lines.push(`        self.wait_for_element_clickable(${finalClickSelector}, timeout=10)`);
        ...
        """, slider)`);
            ...
            case 'Input': { // Text input (supports wrapper -> input redirection)
                if (action.value && selector) {
                    ...
                    lines.push(`        try:`);
                    lines.push(`            self.scroll_to(${qInput}${inputSel}${qInput})`);
                    lines.push(`        except Exception:`);
                    lines.push(`            pass  # Continue even if scroll fails`);
            ...
            case 'Select': { // Native <select> selection
                ...
                if (isNativeSelect) {
                    if (!sval) {
                        ...
                        lines.push(`        try:`);
                        lines.push(`            self.wait_for_element_present(${finalSelector}, timeout=5)`);
            ...
            case 'Checkbox': { // Checkbox: check/uncheck based on final state
                ...
                if (typeof selector === 'string' && selector.startsWith('/') && /\/input(\[\d+\])?$/.test(selector)) {
                    ...
                    console.log(`Background: Detected custom checkbox, adjusted selector from ${selector} to ${checkboxSelector}`);
                }
                
                const checkboxSelectorQuote = checkboxSelector.includes("'") ? '"' : "'";
                const finalCheckboxSelector = `${checkboxSelectorQuote}${checkboxSelector}${checkboxSelectorQuote}`;
                ...
                if (action.needsScrollToEnable && action.scrollAreaSelector) {
                    ...
                    lines.push(`        scroll_area_selector = ${finalCheckboxSelector.includes("'") ? '"' : "'"}${action.scrollAreaSelector}${finalCheckboxSelector.includes("'") ? '"' : "'"}`); 
                    lines.push(`        self.wait_for_scroll_and_enable(scroll_area_selector, ${finalCheckboxSelector})`);
                } else {
                    lines.push(`        try:`);
                    lines.push(`            self.wait_for_element_present(${finalCheckboxSelector}, timeout=5)`);
                    lines.push(`            self.scroll_to(${finalCheckboxSelector})`);
                    lines.push(`        except Exception:`);
                    lines.push(`            pass  # Continue even if scroll fails`);
                    lines.push(`        # Wait for checkbox to be enabled (handle dynamic checkboxes)`);
                    lines.push(`        self.wait_for_element_present(${finalCheckboxSelector}, timeout=10)`);
                    lines.push(`        self.wait_for_element_clickable(${finalCheckboxSelector}, timeout=30)`);
                    
                    if (!isCustomCheckbox) {
                        // Only wait for disabled attribute on native checkboxes
                        lines.push(`        # Additional wait for checkbox to become enabled`);
                        lines.push(`        self.wait_for_attribute_not_value(${finalCheckboxSelector}, 'disabled', timeout=30)`);
                    }
                }
                ...
                if (isCustomCheckbox) {
                    lines.push(`        # Custom checkbox component detected (e.g., Ant Design)`);
                    lines.push(`        # Using JavaScript to check current state and click parent if needed`);
                    lines.push(`        target_checked = ${isChecked ? 'True' : 'False'}`);
                    // Use js_click and execute_script to handle hidden inputs
                    lines.push(`        try:`);
                    lines.push(`            # Get current checkbox state via JavaScript (input is hidden)`);
                    lines.push(`            current_checked = self.execute_script(`);
                    lines.push(`                "return document.evaluate(${finalSelector.replace(/'/g, "\\'")}, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.checked"`);
                    lines.push(`            )`);
                    lines.push(`            if current_checked != target_checked:`);
                    lines.push(`                # Click the visible parent element to toggle`);
                    lines.push(`                self.js_click(${finalCheckboxSelector})`);
                    lines.push(`        except Exception as e:`);
                    lines.push(`            # If JavaScript fails, try direct click on parent`);
                    lines.push(`            self.click(${finalCheckboxSelector})`);
                } else {
                    // Native checkbox: use SeleniumBase methods
                    if (isChecked) lines.push(`        self.check_if_unchecked(${finalCheckboxSelector})`);
                    else lines.push(`        self.uncheck_if_checked(${finalCheckboxSelector})`);
                }
                break;
            }
            ...
            case 'Hover': {
                ...
                lines.push(`        except Exception as e:`);
                lines.push(`            print(f"Hover action skipped for ${finalSelector}: {e}")`);
                break;
            }
            case 'Navigate': {
                // Navigate to a new URL (tab switch or page navigation)
                const url = action.url || action.value || '';
                if (url && url !== startURL) { // Don't repeat the initial URL
        ...
        if (action.value !== undefined && action.value !== null && action.value !== '') {
            ...
            const valueStr = String(action.value).replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, '\\n');
...
function generateChromeRecorderJSON(actionsToUse = null) {
    ...
    for (const action of actions) {
        if (!action || action.type === 'HTML_Capture' || action.type === 'ScreenRecordingStart' || action.type === 'ScreenRecordingStop') {
        ...
        if (action.type === 'Click' && 
            ...
            console.log(`Background: [JSON-GEN] Skipping Click on checkbox/radio selector: ${selector}`);
            continue;
        }
            ...
            case 'Checkbox':
                ...
                if (action.value !== undefined && action.value !== null) {
                    ...
                    try {
                        ...
                        else if (typeof action.value === 'number') isChecked = (action.value === 1);
    ...
    try {
        ...
        flushAllPendingInputs().then(() => {
            ...
            chrome.storage.local.get(['selbas_upload_dir']).then((res) => {
                ...
                const scriptSleepInterval = (htmlCaptureConfig && htmlCaptureConfig.scriptSleepInterval != null) 
                    ? parseFloat(htmlCaptureConfig.scriptSleepInterval) 
                    : 1; // Default 1 second
                
                console.log(`Background: Using script sleep interval: ${scriptSleepInterval} seconds`);
                ...
                async function processBatch(items, batchSize, processFn) {
                    ...
                    for (let i = 0; i < items.length; i += batchSize) {
                        ...
                        const batchResults = await Promise.allSettled(batch.map((item, idx) => processFn(item, i + idx)));
                ...
                const script = generateSeleniumBaseScript({ uploadDir, scriptSleepInterval }, cleanedActions);
                const chromeRecorderJSON = generateChromeRecorderJSON(cleanedActions);
                const zip = new JSZip();
                ...
                const screenshotPromises = capturedScreenshots.map((s, idx) => {
                    ...
                    return fetch(s.dataUrl).then(r => r.arrayBuffer()).then(buf => {
                ...
                const uploadPromises = (Array.isArray(uploadedFiles) ? uploadedFiles : []).map((f) => {
                    try {
                        ...
                        return fetch(f.dataUrl).then(r => r.arrayBuffer()).then(buf => {
                            zip.file(fname, buf);
                ...
                Promise.all([...htmlPromises, ...screenshotPromises, ...uploadPromises])
                    .then(() => {
                        ...
                        chrome.runtime.sendMessage({ command: 'export_progress', data: { current: totalFiles, total: totalFiles, status: 'Generating ZIP file...' } }).catch(() => {});
                    ...
                    .then(blob => {
                        ...
                        reader.onload = function () {
                            ...
                            chrome.runtime.sendMessage({ command: 'export_progress', data: { current: totalFiles, total: totalFiles, status: 'Downloading ZIP file...' } }).catch(() => {});
                            chrome.downloads.download({ url: dataUrl, filename: "seleniumbase_recording.zip", saveAs: true })
                                .then((downloadId) => { 
                                    try { ignoredDownloadIds.add(downloadId); } catch (e) { } 
                                    chrome.runtime.sendMessage({ command: 'export_progress', data: { current: totalFiles, total: totalFiles, status: 'Export completed!' } }).catch(() => {});
                                    sendResponse && sendResponse({ success: true }); 
                                    resetRecordingState(true); 
                                })
                                .catch(err => { 
                                    chrome.runtime.sendMessage({ command: 'export_progress', data: { current: totalFiles, total: totalFiles, status: 'Export failed!' } }).catch(() => {});
                                    sendResponse && sendResponse({ success: false, message: err && err.message ? err.message : String(err) }); 
                                    resetRecordingState(true); 
                                });
                        };
                ...
                chrome.runtime.sendMessage({ command: 'export_progress', data: { current: 0, total: 0, status: 'Fallback export mode...' } }).catch(() => {});
                
                // Use default sleep interval of 1 second in fallback mode
                const script = generateSeleniumBaseScript({ uploadDir: null, scriptSleepInterval: 1 });
                ...
                Promise.all([...htmlPromises, ...screenshotPromises, ...uploadPromises])
                    .then(() => {
                        chrome.runtime.sendMessage({ command: 'export_progress', data: { current: 0, total: 0, status: 'Finalizing export...' } }).catch(() => {});
                        return zip.generateAsync({ type: "blob" });
                    })
                    .then(blob => {
                        const reader = new FileReader();
...
function resetRecordingState(disablePanel = false) {
    ...
    tabsToStop.forEach(tabId => {
        if (tabId) {
            chrome.tabs.sendMessage(tabId, { command: 'stop_recording' })
                .then(() => console.log(`Background: Sent stop_recording to tab ${tabId}`))
                .catch(err => console.warn(`Background: Failed to send stop_recording to tab ${tabId}:`, err));
        }
    });
...
function shouldCaptureHTML(reason, newHTML = null) {
    ...
    if (now - lastCaptureTime < htmlCaptureConfig.minInterval) {
        console.log(`HTML capture skipped: too soon (${now - lastCaptureTime}ms < ${htmlCaptureConfig.minInterval}ms)`);
        return false;
    }
    ...
    if (captureTimestamps.length >= htmlCaptureConfig.maxCapturesPerMinute) {
        console.log(`HTML capture skipped: rate limit (${captureTimestamps.length}/${htmlCaptureConfig.maxCapturesPerMinute} per minute)`);
        return false;
    }
    ...
    if (htmlCaptureConfig.mode === 'smart' && newHTML && lastCapturedHTML) {
        ...
        if (similarity >= htmlCaptureConfig.similarityThreshold) {
            console.log(`HTML capture skipped: too similar (${(similarity * 100).toFixed(1)}% >= ${(htmlCaptureConfig.similarityThreshold * 100).toFixed(1)}%)`);
            return false;
        }
        console.log(`HTML capture allowed: significant change (${(similarity * 100).toFixed(1)}% similarity)`);
    }
        ...
        chrome.tabs.sendMessage(targetTabId, { command: "get_html" }, (response) => {
            ...
            if (response && response.success && typeof response.html === 'string') {
                ...
                if (!shouldCaptureHTML(reason, response.html)) {
                    console.log(`Background: HTML capture skipped for reason: ${reason}`);
                ...
                console.log(`Background: HTML captured from tab ${targetTabId} for reason: ${reason} (${capturedHTMLs.length} total).`);
                resolve();
            } else {
...
function flushPendingInput(selector) { // Flush pending Input for specific selector into action list (deduplicate, keep latest)
    ...
    return Promise.allSettled([triggerScreenshot(), triggerHTMLCapture(null, 'input_flush')]) // Also capture screen and HTML
        .finally(() => {
...
async function ensureContentScriptInTab(tabId) { // Ensure content.js is injected into specified tab
    try {
        ...
        if (isRecording) {
            ...
            chrome.tabs.sendMessage(tabId, { command: 'start_recording' })
                .then(() => console.log(`Background: Sent start_recording to tab ${tabId}`))
                .catch(err => console.warn(`Background: Failed to send start_recording to tab ${tabId}:`, err));
        }
    } catch (e) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => { // Main message handler: receive popup/content/sidepanel commands
    ...
    switch (cmd) {
        case "start_recording": { // Start recording
            ...
            console.log(`Background: Cross-tab recording enabled - you can switch tabs or open new windows during recording`);
            ...
            chrome.scripting.executeScript({ target: { tabId: recordingTabId }, files: ['content.js'] })
                ...
                .then(() => {
                    ...
                    return chrome.tabs.sendMessage(recordingTabId, { command: 'start_recording' })
                        .catch(err => console.warn('Background: Failed to send start_recording to content script:', err));
                })
        ...
        case "record_action": {
            ...
            if (action.type === 'Hover') {
                ...
                console.log('Background: Current recordedActions count:', recordedActions.length);
            }
            ...
            if (action.type === 'Input' || action.type === 'Hover') {
                ...
                if (action.type === 'Hover') {
                    console.log('Background: 🎯 Processing Hover action, adding to recordedActions');
                }
                ...
                if (action.type === 'Hover') {
                    console.log('Background: ✅ Hover action added! Total actions now:', recordedActions.length);
                    console.log('Background: Last action in array:', recordedActions[recordedActions.length - 1]);
                }
            ...
            const processNonInputAction = (actionToRecord) => {
                ...
                const delayMs = 0;
                setTimeout(() => {
                    const tasks = [triggerHTMLCapture(senderTabId || recordingTabId, 'user_action')];
        ...
        case "replace_action": { // Replace selector/value for a specific action
            const { stepNumber, selector, value, tagName, elementType } = message.data || {};
            if (typeof stepNumber !== 'number' || stepNumber < 1 || stepNumber > recordedActions.length) {
            ...
            if (recordedActions[actionIndex]) {
                ...
                if (selector !== undefined) {
                    ...
                    console.log(`Background: Updated selector for step ${stepNumber}: "${selector}"`);
                }
                
                // Update value if provided
                if (value !== undefined) {
                    recordedActions[actionIndex].value = value;
                    console.log(`Background: Updated value for step ${stepNumber}: "${value}"`);
                }
                ...
                if (tagName) {
                    ...
                    if (newType !== recordedActions[actionIndex].type) {
                        ...
                        console.log(`Background: Updated action type for step ${stepNumber}: "${newType}"`);
                    }
                }
        ...
        case "update_action_comment": { // Update comment for a specific action
            ...
            if (recordedActions[actionIndex]) {
                ...
                console.log(`Background: Updated comment for step ${stepNumber}: "${comment}"`);
        ...
        case "save_export": { // User pressed Save & Export
            ...
            console.log(`Background: Save export accepted - ${recordedActions.length} actions, isRecording=${isRecording}`);
        ...
        case "cancel_recording": { // Cancel entire recording
        ...
        case "capture_html": { // Side panel request: Manual capture of current HTML
        ...
        case "update_html_capture_config": { // Update HTML capture configuration
            if (message.config) {
                htmlCaptureConfig = { ...htmlCaptureConfig, ...message.config };
                console.log('Background: HTML capture config updated:', htmlCaptureConfig);
                sendResponse({ success: true });
            } else {
        ...
        case "get_status": { // Reply with recording status (to popup or other)
            sendResponse({ 
                isRecording, 
                recordingTabId,
        ...
        case "get_recording_state": { // Get current recording state (actions and counts)
            sendResponse({ 
                actions: recordedActions, 
                htmlCount: capturedHTMLs.length,
        ...
        case "open_editor": { // Open side panel in editor mode
        ...
        case "import_actions": { // Import actions from JSON file
        ...
        case "request_current_state": { // Side panel initialization request current state
            console.log("Background: Received request_current_state from", sender.contextType);
            console.log("Background: isRecording =", isRecording, "recordedActions.length =", recordedActions.length);
            ...
            if (isRecording) {
                ...
                console.log("Background: Sending state with", response.actions.length, "actions");
        ...
        case "stop_recording_internal": { // Side panel notifies stop before closing (e.g., browser X button)
            if (isRecording && sender.contextType === "SIDE_PANEL") {
                console.log("Background: Side panel closed, resetting recording state completely");
        ...
        case "popup_opened": { // content.js notification: Detected window.open / popupWindow
            if (!isRecording) { sendResponse({ success: false, message: "Not recording." }); return true; }
...
chrome.tabs.onCreated.addListener((tab) => {
    try {
        ...
        const openerMatches = tab.openerTabId && allowedRecordingTabs.has(tab.openerTabId) && sinceLastClick <= NEW_TAB_DETECT_MS;
        ...
        const shouldTrack = openerMatches || expectingMatches || (tab.openerTabId && allowedRecordingTabs.has(tab.openerTabId));
        ...
        if (shouldTrack) {
            ...
            console.log(`Background: Detected new tab ${tab.id} opened (pending capture). Opener: ${tab.openerTabId}`, pendingNewTabs[tab.id]);
            ...
            console.log(`Background: New tab ${tab.id} created during recording, adding to allowed tabs`);
            allowedRecordingTabs.add(tab.id);
        }
...
chrome.tabs.onActivated.addListener(async (activeInfo) => {
    try {
        ...
        console.log(`Background: Tab activated: ${tabId} (recording: ${isRecording})`);
        ...
        chrome.tabs.get(tabId, (tab) => {
            ...
            if (lastRecordedURL && currentURL !== lastRecordedURL && !currentURL.startsWith('chrome://')) {
                console.log(`Background: 🌐 URL changed from ${lastRecordedURL} to ${currentURL}`);
...
chrome.runtime.onConnect.addListener((port) => {
    ...
    if (port.name === 'sidepanel') {
        ...
        port.onDisconnect.addListener(async () => {
            console.log('Background: Side panel disconnected - checking if recording needs to be stopped');
            console.log('Background: isRecording =', isRecording, 'recordingTabId =', recordingTabId);
            
            if (isRecording && recordingTabId) {
                console.log('Background: Recording was active, showing confirmation dialog');
                ...
                try {
                    ...
                    if (userConfirmed) {
                        ...
                        console.log('Background: Recording state preserved - user can reopen side panel to continue');
                    }
                } catch (error) {
                    console.error('Background: ❌ Error showing confirmation dialog:', error);
                ...
                console.log('Background: Not recording or no recording tab, ignoring disconnect');
            }
        });
    }
});