...
(function() {
    if (window.__SELBAS_CONTENT_SCRIPT_LOADED__) {
        console.log("Selenium Recorder: Content script already loaded, skipping re-execution.");
        return;
    }
    window.__SELBAS_CONTENT_SCRIPT_LOADED__ = true;

    console.log("Selenium Recorder: Content script injected (v13 - improved class selectors)."); // Version info for debugging

// --- Inject API interceptor into page context (fetch + XHR) ---
(function injectApiInterceptor() { // Intercept fetch/XHR and forward API information to background
    function serializeBody(body) { // Convert request body to recordable string
        try {
            if (!body) return null;
    ...
    function shouldIgnoreUrl(url) { // Filter out ad domains and tracking URLs to reduce noise
    ...
    window.fetch = async function (input, init) {
        ...
        try {
            ...
            window.postMessage({ // Send API results back to content script via postMessage
            ...
            if (!errStr.includes('ERR_BLOCKED_BY_CLIENT') && 
    ...
    try { window.XMLHttpRequest = ProxyXHR; } catch (e) { /* Some pages may prohibit overwriting */ }
})();

// --- Inject dialog (alert/prompt/confirm) capture ---
(function injectDialogHooks() { // Intercept alert/confirm/prompt to record dialog events
    try {
        const script = document.createElement('script');
        script.src = chrome.runtime.getURL('dialog_hooks.js');
        script.onload = () => script.remove();
        script.onerror = () => {
            console.warn('Failed to load dialog_hooks.js, falling back to direct injection');
            script.remove();
        };
 ...
 */
function generateAbsoluteXPath(element) { // Generate absolute XPath (final fallback) with enhanced error handling
    ...
    while (currentElement && currentElement.nodeType === Node.ELEMENT_NODE && iterationCount < MAX_ITERATIONS) {
        iterationCount++;
...
function xpathLiteral(s) { // Safely wrap string as XPath literal (handle quotes)
    if (s == null) return "''";
    const str = String(s);
...
function trimNonInteractiveXPathTail(xp) { // Trim non-interactive nodes from XPath tail (like svg/path)
    try {
...
function isWeakSelector(sel) { // Determine if selector is very fragile (to avoid)
    try {
 ...
 */
function generateRobustSelector(el) { // Generate more robust selector (prioritize stable attributes/classes, then fallback to XPath)
    if (!(el instanceof Element)) return null;
    const originalEl = el;
    ...
    // 1. ID (preferred if stable and unique)
    if (el.id) {
        const id = el.id;
        // Relax stable ID conditions, only exclude obvious framework-generated IDs
        const unstableIdRegex = /^(?:radix-|ember-|data-v-|svelte-|ui-id-|aria-|temp-|auto-|react-)/i;
        ...
        if (!looksUnstable) {
            try {
                ...
                if (document.querySelectorAll(cssSelector).length === 1) {
                    console.log(`✅ Using stable ID selector: ${cssSelector}`);
                    return cssSelector;
                }
            } catch (e) { /* ignore */ }

            // XPath version as fallback
            try {
                ...
                if (document.evaluate(xpathSelector, document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength === 1) {
                    console.log(`✅ Using ID XPath selector: ${xpathSelector}`);
    ...
    // 4. data-dnd-kit-id (stable ID from DND-Kit drag library)
    const dndKitId = el.getAttribute('data-dnd-kit-id');
    if (dndKitId) {
        // Prefer CSS selector
        ...
        // Also provide XPath version as fallback
        const xpathSelector = `//div[@data-dnd-kit-id="${dndKitId}"]`;
        try {
            if (document.evaluate(xpathSelector, document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength === 1) {
                console.log(`DND-Kit XPath selector: ${xpathSelector}`);
                return xpathSelector;
            }
        } catch (e) { /* ignore */ }
    }
    ...
    // 6.5 If promoted to clickable ancestor, try ancestor-descendant related XPath (e.g., icons with title/aria)
    try {
 ...
 */
function getElementInfo(element) { // Extract basic element information (for UI display and debugging)
...
async function sendActionToAPI(element, actionType, actionData) {
    try {
        ...
        try {
            ...
            if (computedStyle) {
                ...
                importantStyleProps.forEach(prop => {
                    ...
                    if (value && value !== 'auto' && value !== 'normal' && value !== 'inherit') {
        ...
        // 發送到用戶的 API 端點
        const response = await fetch('http://127.0.0.1:5000/api/create_new_element', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            console.log('[API] ❌ Failed to send action data:', response.status, response.statusText);
        }
 ...
 */
function getClosestAnchorSelector(el) { // Try to find nearest anchor tag and generate selector
    try {
 ...
 * @param {Event} event
 */
function handleClick(event) { // Handle click events; if hitting icon/svg, find clickable ancestor (using absolute XPath)
    try {
        ...
        console.log(`[CLICK-DEBUG] Raw target: ${rawTarget.tagName} (${rawTarget.className})`);
        console.log(`[CLICK-DEBUG] Element at point: ${elementAtPoint ? elementAtPoint.tagName : 'null'} (${elementAtPoint ? elementAtPoint.className : 'null'})`);
        
        let actualTarget = rawTarget;
        
        // PRIORITY 0: Ant Design Select options - use data-* attributes and text content
        if (elementAtPoint) {
            const antSelectItem = elementAtPoint.closest('.ant-select-item, .ant-select-item-option, [class*="rc-select-item"]');
            if (antSelectItem) {
                console.log(`[CLICK] 🎯 ANT DESIGN SELECT ITEM detected:`, antSelectItem);
            ...
            const antRadioWrapper = elementAtPoint.closest('.ant-radio-button-wrapper, .ant-radio-wrapper');
            if (antRadioWrapper) {
                // Find the actual radio input inside the wrapper
                const radioInput = antRadioWrapper.querySelector('input[type="radio"]');
                if (radioInput) {
                    console.log(`[CLICK] 🎯 ANT DESIGN RADIO BUTTON detected:`, radioInput);
                    console.log(`[CLICK] Radio value:`, radioInput.value);
                    console.log(`[CLICK] Radio wrapper text:`, antRadioWrapper.textContent.trim());
                    actualTarget = radioInput;
                } else {
                    console.log(`[CLICK] ⚠️ ANT DESIGN RADIO WRAPPER found but no input:`, antRadioWrapper);
        ...
        if (elementAtPoint && elementAtPoint.tagName && elementAtPoint.tagName.toLowerCase() === 'li') {
            const parentList = elementAtPoint.closest('ul, ol');
            if (parentList && (rawTarget === parentList || parentList.contains(rawTarget))) {
                console.log(`[CLICK] 🎯 FORCE using LI instead of container:`, elementAtPoint);
        ...
        // PRIORITY 1.5: If rawTarget is inside an LI, use the LI instead
            ...
            console.log(`[CLICK] 🎯 FOUND LI ancestor for ${rawTarget.tagName}:`, liElement);
            actualTarget = liElement;
        }
        // PRIORITY 2: If rawTarget is UL/OL and elementAtPoint is different, prefer elementAtPoint
        else if (rawTarget.tagName && ['ul', 'ol'].includes(rawTarget.tagName.toLowerCase()) && 
                 elementAtPoint && elementAtPoint !== rawTarget && rawTarget.contains(elementAtPoint)) {
            console.log(`[CLICK] 🎯 REPLACING ${rawTarget.tagName} with ${elementAtPoint.tagName}:`, elementAtPoint);
            actualTarget = elementAtPoint;
        }
        // PRIORITY 3: General container replacement logic
            ...
            const isContainerElement = rawTarget.tagName && 
                ['ul', 'ol', 'nav', 'menu', 'section', 'article', 'header', 'footer', 'aside'].includes(rawTarget.tagName.toLowerCase());
            
            // Check if elementAtPoint is a more specific interactive element
            const isMoreSpecific = elementAtPoint.tagName && 
                ['li', 'a', 'button', 'span', 'div', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'label', 'td', 'th'].includes(elementAtPoint.tagName.toLowerCase());
            
            if (isContainerElement && isMoreSpecific && rawTarget.contains(elementAtPoint)) {
                console.log(`[CLICK] ✅ Refining container ${rawTarget.tagName} to ${elementAtPoint.tagName}:`, elementAtPoint);
                actualTarget = elementAtPoint;
            }
        }
        
        console.log(`[CLICK-FINAL] Using target: ${actualTarget.tagName} (${actualTarget.className})`);
        
        // Store if we deliberately chose a specific target (like LI over UL)
        const wasTargetRefined = actualTarget !== rawTarget;

        const clickableAncestor = actualTarget.closest && actualTarget.closest('button, a[href], [role="button"], .ant-btn, [onclick], [tabindex]:not([tabindex="-1"]), [role="combobox"], [aria-haspopup="listbox"], [class*="__control"], [class*="__value-container"], [class*="auto-complete"]');
        const anchorEl = clickableAncestor ? (clickableAncestor.closest && clickableAncestor.closest('a')) : (actualTarget.closest ? actualTarget.closest('a') : null);
        
        // CRITICAL: If we refined the target (e.g., LI from UL), don't let clickableAncestor override it
        let targetForSelector;
        if (wasTargetRefined && clickableAncestor && 
            ['ul', 'ol', 'nav', 'menu'].includes(clickableAncestor.tagName.toLowerCase())) {
            console.log(`[CLICK] 🚫 Preventing clickableAncestor override: keeping ${actualTarget.tagName} instead of ${clickableAncestor.tagName}`);
        ...
        try {
            ...
            if (targetForSelector.matches && targetForSelector.matches('.ant-select-item, .ant-select-item-option, [class*="rc-select-item"]')) {
                ...
                console.log(`[CLICK] 🎯 Generating selector for Ant Design Select option:`, optionText);
            ...
            // Special handling for Ant Design Radio buttons
            else if (targetForSelector.matches && targetForSelector.matches('input[type="radio"]') && 
                     targetForSelector.closest && targetForSelector.closest('.ant-radio-button-wrapper, .ant-radio-wrapper')) {
                const radioValue = targetForSelector.value;
                const wrapper = targetForSelector.closest('.ant-radio-button-wrapper, .ant-radio-wrapper');
                const labelText = wrapper ? wrapper.textContent.trim() : '';
                
                console.log(`[CLICK] 🎯 Generating selector for Ant Design Radio button:`, radioValue, labelText);
                ...
                if (radioValue !== undefined && radioValue !== '') {
                    ...
                    console.log(`[CLICK] ✅ Using label-text-based selector: ${selector}`);
                } else {
                    // Fallback to absolute XPath
                    const abs = generateAbsoluteXPath(targetForSelector);
                    selector = abs ? trimNonInteractiveXPathTail(abs) : null;
 ...
 * @param {Event} event
 */
function handleChange(event) { // Listen to change: handle select/file/checkbox (using absolute XPath; text input handled by input events)
    ...
    // <input type="checkbox">: record final checked state
        ...
        try {
            ...
            const specificScrollAreas = document.querySelectorAll('.scroll-area, #scrollArea, [class*="scroll"], .scroll-container');
            for (const container of specificScrollAreas) {
                if (container.scrollHeight > container.clientHeight) {
                    ...
                    const isBelow = checkboxRect.top > containerRect.bottom && checkboxRect.top < containerRect.bottom + 200;
                    const horizontalOverlap = !(containerRect.right < checkboxRect.left || containerRect.left > checkboxRect.right);
                    
                    if (isBelow && horizontalOverlap) {
                        needsScrollToEnable = true;
                        scrollArea = container;
                        console.log('[CHECKBOX] Detected scroll-to-enable pattern with specific scroll area:', container);
                        break;
                    }
                }
            }
            ...
            if (!needsScrollToEnable) {
                ...
                for (const container of scrollContainers) {
                    ...
                    if (style.overflowY === 'auto' || style.overflowY === 'scroll') {
                        ...
                        const verticalDistance = Math.abs(containerRect.bottom - checkboxRect.top);
                        const horizontalOverlap = !(containerRect.right < checkboxRect.left || containerRect.left > checkboxRect.right);
                        
                        if (verticalDistance < 100 && horizontalOverlap) {
                            needsScrollToEnable = true;
                            scrollArea = container;
                            console.log('[CHECKBOX] Detected scroll-to-enable pattern with overflow container:', container);
                            break;
                        }
                    }
                }
        ...
        actionData = {
            ...
            scrollAreaSelector: scrollArea ? (() => {
                try {
                    ...
                    return scrollAbs ? trimNonInteractiveXPathTail(scrollAbs) : null;
                } catch (e) { return null; }
            })() : null
        };
    }
...
function handleSliderPointerUp(event) { // Handle custom ARIA sliders (common in UI libraries)
    try {
 ...
 */
function handleInputEvent(event) { // Only handle real-time input for autocomplete inputs, regular inputs use blur events
    try {
        ...
        const isTextLikeInput = (tag === 'input' && (/^(text|password|search|email|url|tel|number)$/i.test(type))) 
            || (tag === 'textarea')
        ...
        const action = { // Send to background page, ask it to integrate final value with debounce
            type: 'Input',
 ...
 */
function isAutocompleteInput(el) { // Determine if it's an autocomplete input
    try {
 ...
 * @param {Event} event
 */
function handleBlurEvent(event) { // Handle blur events for regular input fields, record final value
    try {
        const el = event.target;
        if (!(el instanceof Element)) return;
        ...
        const role = (el.getAttribute('role') || '').toLowerCase();
        const isTextLikeInput = (tag === 'input' && (/^(text|password|email|url|tel|number)$/i.test(type))) 
            || (tag === 'textarea') 
            || (tag === 'input' && role === 'spinbutton')
            || (tag === 'input' && el.classList.contains('ant-input-number-input'));
        ...
        let selector = null;
        try {
            if (el.classList.contains('ant-input-number-input') || role === 'spinbutton') {
                // Try to find Ant Design InputNumber wrapper
                const wrapper = el.closest('.ant-input-number, .ant-input-number-input-wrap');
                if (wrapper && wrapper.parentElement) {
                    selector = generateAbsoluteXPath(wrapper.parentElement);
                    console.log('[Input] Using Ant InputNumber wrapper for selector:', selector);
                } else {
                    selector = generateAbsoluteXPath(el);
                }
            } else {
...
function handleCompositionStart(event) { // IME composition start: mark state, pause recording intermediate input
    try {
        const el = event.target;
        if (!(el instanceof Element)) return;
        isComposingIME = true;
...
function handleCompositionEnd(event) { // IME composition end: send final value
    try {
        const el = event.target;
...
window.addEventListener('message', (event) => { // Forward API interception info to background page
...
window.addEventListener('message', (event) => { // Forward dialog events to background page
...
function serializeElementForNotification(el) { // Use absolute XPath for consistency with better error handling
    try {
...
function handleMutations(mutationsList) { // Batch collect new elements and notify background with better filtering
            ...
            for (const node of mut.addedNodes) {
                if (node.nodeType === Node.ELEMENT_NODE) {
                    ...
                    if (node.isConnected && document.contains(node)) {
                        ...
                        if (tagName && !['script', 'style', 'meta', 'link', 'noscript', 'head'].includes(tagName)) {
                            added.push(node);
                        }
                    }
...
function attachListeners() { // Attach various event listeners and MutationObserver
...
function detachListeners() { // Remove event listeners and observer
    if (!isListenerAttached) return;
...
function disableRecordingHighlight() {
    ...
    try {
        const highlightedElements = document.querySelectorAll('.__recording_highlight__');
        highlightedElements.forEach(el => {
            el.classList.remove('__recording_highlight__');
        });
    } catch (e) {
        console.warn('[Recording Highlight] Failed to clean up all highlights:', e);
    }
    ...
    try {
        const styleElement = document.getElementById('__recording_highlight_styles__');
        if (styleElement) {
            styleElement.remove();
        }
    } catch (e) {
...
function handleRecordingHighlight(e) {
    ...
    if (targetElement && 
        ...
        targetElement.nodeType === 1) { // Element node
...
function startElementPicker(responseCallback) {
    ...
    const handleMouseMove = (e) => {
        ...
        if (targetElement && targetElement !== document.body && targetElement !== document.documentElement) {
            ...
            const classes = targetElement.className ? `.${targetElement.className.trim().split(/\s+/).join('.')}` : '';
...
function stopElementPicker() {
    ...
    if (overlay) {
        ...
        if (handlers) {
            overlay.removeEventListener('mousemove', handlers.handleMouseMove, true);
...
// --- Message Listener ---
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => { // Receive background requests and respond (extract HTML, check selectors, etc.)
    // console.log("Content script received message:", message.command); // Optional debug
    
    if (message.command === "start_recording") {
        console.log("[Content] Received start_recording command, attaching listeners...");
        attachListeners();
    ...
    if (message.command === "stop_recording") {
        console.log("[Content] Received stop_recording command, detaching listeners...");
        detachListeners();
        sendResponse({ success: true });
        return true;
    }
...
(function injectPopupWindowAndOpenHook() { // Inject window.open and popupWindow hooks to help detect new tabs/popups
    try {
        const script = document.createElement('script');
        script.src = chrome.runtime.getURL('popup_hooks.js');
        script.onload = () => script.remove();
        script.onerror = () => {
            console.warn('Failed to load popup_hooks.js, falling back to direct injection');
            script.remove();
        };
...
(function removeLegacyScreenRecorder() { // Clean up legacy floating screen recorder button from older versions
    try {
        ...
        while (current && current !== document.body) {
            if (current.id && !current.id.match(/^(?:radix-|ember-|data-v-|svelte-|ui-|aria-|temp-|auto-)/i)) {
                ...
                try {
                    ...
                    if (document.querySelectorAll(selector).length === 1) {
                        console.log(`[DND][resolveContainer] Found stable ID container: ${selector}`);
...
function handleDragEnter(event) {
    try {
        ...
        const hasFileTypes = event.dataTransfer && event.dataTransfer.types && event.dataTransfer.types.includes('Files');
        const hasRecordedSource = window.__SELBAS_DRAG_SOURCE__ && window.__SELBAS_DRAG_SOURCE__.selector;
        const isExternalFileDrag = hasFileTypes && !hasRecordedSource;
        
        if (isExternalFileDrag) {
            event.preventDefault();
            console.log('[DND][dragenter] External file drag entering:', event.target);
            ...
            console.log('[DND][dragenter] Internal element drag with Files type detected, not treating as file upload');
        }
    } catch (e) {
...
function handleFileDrop(event) {
    try {
        ...
        if (hasRecordedSource) {
            ...
            const hasUploadForm = rawDropTarget.closest('form') && rawDropTarget.closest('form').querySelector('input[type="file"]');
            const hasExplicitUploadIndicators = rawDropTarget.closest('[data-file-upload], [data-upload], input[type="file"]') ||
                                               /file.*upload|upload.*file|file.*input/i.test(rawDropTarget.textContent || '') ||
                                               rawDropTarget.hasAttribute('accept');
            
            if (!hasFileInput && !hasUploadForm && !hasExplicitUploadIndicators) {
                console.log('[DND][file-drop] Skipping file drop handling - appears to be internal element drag with phantom files');
                return;
            }
            
            console.log('[DND][file-drop] Confirmed file upload context despite recorded drag source');
        }
        ...
        let selector = null;
        try {
            // Look for file upload related elements more specifically
            ...
            // First try to find a file input near the drop target
            for (const sel of uploadSelectors) {
                // Check the target itself
                if (rawDropTarget.matches && rawDropTarget.matches(sel)) {
                    fileInput = rawDropTarget;
                    break;
                }
                
                // Check within the target
                fileInput = rawDropTarget.querySelector(sel);
                if (fileInput) break;
                
                // Check in parent elements
                let parent = rawDropTarget.parentElement;
                while (parent && parent !== document.body) {
                    fileInput = parent.querySelector(sel);
                    if (fileInput) break;
                    parent = parent.parentElement;
                }
                if (fileInput) break;
            ...
            if (!fileInput) {
                console.log('[DND][file-drop] No file input found - not recording as upload');
                return;
            }
            ...
            console.log(`[DND][file-drop] Using file input element for selector:`, selector);
        } catch (e) {
...
function handleDrop(event) {
    try {
        ...
        if (hasFiles && hasRecordedSource) {
            ...
            const realFiles = Array.from(event.dataTransfer.files).filter(f => f && f.size >= 0);
            isActualFileUpload = realFiles.length > 0 && realFiles.some(f => f.size > 0 || f.type);
        }
        
        if (isActualFileUpload) {
            console.log('[DND][file-drop] Detected actual file drop with', event.dataTransfer.files.length, 'files');
            handleFileDrop(event);
            return; // File drop is handled separately from element drag-and-drop
        } else if (hasFiles && hasRecordedSource) {
            console.log('[DND][element-drag] Detected element drag with phantom files, treating as element drag');
        }
...
function handleDragOver(event) {
    try {
        ...
        if (isExternalFileDrag) {
            ...
            console.log('[DND][dragover] Internal element drag over (with Files type):', event.target);
        }
...
(function attachHoverDetection() {
    ...
    document.addEventListener('mouseover', (event) => {
        try {
            ...
            const isInteractive = target.matches('a, button, [role="button"], [role="combobox"], [aria-haspopup], [data-testid*="trigger"], [class*="dropdown"], [class*="menu"], [onclick], [onmouseover], [onmouseenter]');
            ...
            const timerId = setTimeout(() => {
                ...
                if (now - lastTs < DEDUPE_MS) {
                    console.log('[HOVER-DEBUG] ❌ Duplicate hover detected (within dedupe window), skipping');
                    return;
                }
                lastHoverBySelector.set(selector, now);

                console.log('[HOVER-DEBUG] ✅ Recording hover action, sending to background...');
                try {
                    chrome.runtime.sendMessage({
                    ...
                    console.log('[HOVER-DEBUG] ✅ Message sent to background successfully!');
                } catch (e) { 
                    console.log('[HOVER-DEBUG] ❌ Error sending message to background:', e);
                }
            }, HOVER_THRESHOLD_MS);
