...
if (typeof window.sidePanelInitialized === 'undefined') {
    ...
    port.onDisconnect.addListener(() => {
        ...
        if (isRecording && !userConfirmedClose) {
            console.log('Side Panel: Recording was active but not confirmed - notifying background');
        }
    });
    ...
    function renderActions(actions = []) { // Render recorded actions to the list
        ...
        actions.forEach(action => {
            ...
            if (action.type === 'HTML_Capture') {
                ...
                // Default to XPath if selectorType is not specified
                const selectorType = action.selectorType || 'XPath';
                detailText = `${selectorType}: ${action.selector}`;
            }
            ...
            if (action.value && action.type !== 'HTML_Capture' && action.type !== 'DragAndDrop') {
                ...
                if (action.type === 'Input' && action.elementInfo && action.elementInfo.textContent) {
                    displayValue = action.elementInfo.textContent.trim();
                } else if (typeof action.value === 'string' && action.value.length > 50) {
            ...
            if (action.type !== 'HTML_Capture') {
                ...
                if (isEditorMode && action.type !== 'Navigate' && action.type !== 'DragAndDrop') {
                ...
                if (action.comment && action.comment.trim()) {
                    ...
                    editButton.title = `Edit comment for step ${action.step}: "${action.comment.trim()}"`;
                }
        ...
        console.log(`[SidePanel] Action count changed: ${lastActionCount} -> ${currentActionCount}`);
       
        if (currentActionCount > lastActionCount) {
            console.log(`[SidePanel] New actions detected, forcing scroll to bottom for ALL action types`);
            // Use setTimeout to ensure DOM is updated before scrolling
            setTimeout(() => {
                const actionsContainer = document.querySelector('.actions-container');
                if (actionsContainer) {
                    console.log(`[SidePanel] Scroll container found, forcing scroll regardless of user activity`);
                    
                    // ALWAYS scroll to bottom for ANY new action - no conditions!
                    actionsContainer.scrollTop = actionsContainer.scrollHeight;
                    console.log(`[SidePanel] FORCED scroll to bottom - scrollTop set to: ${actionsContainer.scrollHeight}`);
    ...
    function updateUI(data) { // Update overall UI state based on data provided by background page
        ...
        if (isEditorMode) {
            ...
            console.log(`[Editor Mode] updateUI: actions=${data.actions?.length}, saveButton.disabled=${saveButton.disabled}`);
    ...
    function handleReplaceAction(action) {
        ...
        if (action.selector) currentInfo += `Current Selector:\n${action.selector}\n\n`;
        ...
        const sendReplaceActionWithElementInfo = (stepNumber, selector, value, elementInfo) => {
            chrome.runtime.sendMessage({
                command: 'replace_action',
                data: {
                    stepNumber: stepNumber,
                ...
                if (response && response.success) {
                    console.log(`Action replaced for step ${stepNumber} with element info`);
                    // Refresh the UI
                    chrome.runtime.sendMessage({ command: 'get_recording_state' });
                } else {
                    console.error('Failed to replace action:', response);
        ...
        if (choice === '1' || choice === '3') {
            ...
            alert('Click on the element you want to select on the page, or press ESC to cancel.');
            ...
            chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                ...
                chrome.tabs.sendMessage(activeTabId, { command: 'start_element_picker' }, (response) => {
                    if (chrome.runtime.lastError) {
                        console.error('Failed to start element picker:', chrome.runtime.lastError);
                    ...
                    if (response && response.success) {
                        ...
                        sendReplaceActionWithElementInfo(action.step, newSelector, newValue, elementInfo);
    ...
    function handleEditComment(stepNumber, currentComment) {
        const newComment = prompt(`Add comment for step ${stepNumber}:`, currentComment);
    ...
    function handleDeleteAction(step, actionType, recordingId) { // Delete specified step (confirm first when deleting screen recording markers)
        ...
        const confirmMsg = actionType === 'ScreenRecordingStart' || actionType === 'ScreenRecordingStop' 
        ...
        chrome.runtime.sendMessage({ command: 'delete_action', data: payload })
            .then(() => {
                ...
                if (isEditorMode && actionCountBadge) {
                    ...
                    chrome.runtime.sendMessage({ command: 'get_recording_state' }).then(response => {
    ...
    function handleSave() { // Trigger save and export ZIP (background script generates Python script and packages)
        ...
        console.log("✅ Side Panel: Save & Export button clicked - proceeding with save");
        ...
        try {
            ...
            for (const tab of tabs) {
                try {
                    ...
                    console.log(`Could not disable side panel for tab ${tab.id}:`, err.message);
                }
            }
    ...
    if (actionsContainer) {
        actionsContainer.addEventListener('scroll', () => {
            ...
            const isNearBottom = actionsContainer.scrollTop + actionsContainer.clientHeight >= actionsContainer.scrollHeight - 50;
            if (isNearBottom) {
                isUserScrolling = false;
            }
        });
    ...
    const modalCaptureFrequency = document.getElementById('modal-capture-frequency');
    ...
    function openSettingsModal() {
        ...
        if (scriptSleepInterval) {
            scriptSleepInterval.value = String(htmlCaptureSettings.scriptSleepInterval != null ? htmlCaptureSettings.scriptSleepInterval : 1);
        }
    ...
    function saveSettings() {
        ...
        if (htmlCaptureSettings.mode !== 'manual') {
            switch (modalCaptureFrequency.value) {
                case 'minimal': htmlCaptureSettings.minInterval = 2000; break;
                case 'normal': htmlCaptureSettings.minInterval = 1000; break;
                case 'frequent': htmlCaptureSettings.minInterval = 500; break;
            }
        }
        ...
        if (scriptSleepInterval) {
            ...
            htmlCaptureSettings.scriptSleepInterval = !isNaN(parsedValue) ? parsedValue : 1;
        }
    ...
    function importPythonScript(scriptContent, fileName) {
        try {
            ...
            console.log(`Side Panel: Extracted ${importedActions.length} actions from Python script`);
            ...
            chrome.runtime.sendMessage({
                ...
                if (response && response.success) {
                    ...
                    console.log('Side Panel: Editor mode enabled, Save button disabled =', saveButton.disabled);
                } else {
    ...
    function parsePythonLine(line, stepNumber) {
        ...
        if (line.includes('.send_keys(')) {
            const textMatch = line.match(/\.send_keys\(["']([^"']+)["'],\s*["']([^"']+)["']/);
            if (textMatch) {
                action.type = 'Input';
            ...
            const keysMatch = line.match(/\.send_keys\(["']([^"']+)["'],\s*Keys\.(\w+)/);
            if (keysMatch) {
        ...
        if (line.includes('.select_option_by_text(')) {
            const match = line.match(/\.select_option_by_text\(["']([^"']+)["'],\s*["']([^"']+)["']/);
            if (match) {
                action.type = 'Select';
                action.selector = match[1];
                action.value = match[2];
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        chrome.storage.local.get(['pendingEditorMode', 'editorModeData'], (result) => {
